...
def main():
    ...
    classes and applying specific decorators to improve developer experience.
    """
    ...
    content = add_extra_items_to_addon_commands(content)
...
def add_extra_items_to_addon_commands(content: str) -> str:
    """
    Adds the @extra_items(Any) decorator to the AddOnCommand TypedDicts
    to allow them to carry arbitrary data.
    """
    print("⚙️  Step 4: Decorating AddOn command TypedDicts to allow extra keys...")

    # Target the AddOnCommandParameters definition
    target_params = "class AddOnCommandParameters(TypedDict):"
    replacement_params = f"@extra_items(Any)\n{target_params}"
    if target_params in content:
        content = content.replace(target_params, replacement_params, 1)
        print("    - Decorated AddOnCommandParameters.")

    # Target the AddOnCommandResponse definition
    target_response = "class AddOnCommandResponse(TypedDict):"
    replacement_response = f"@extra_items(Any)\n{target_response}"
    if target_response in content:
        content = content.replace(target_response, replacement_response, 1)
        print("    - Decorated AddOnCommandResponse.")
...
def assemble_final_file(content: str) -> str:
    ...
    print("⚙️  Step 5: Assembling final file...")
    ...
    DECORATOR_PLACEHOLDER = "###DECORATOR_NEWLINE_PROTECTION###"

    full_content = re.sub(
        r"(@[^\n]+)\n(?=class\s)",
        r"\1" + DECORATOR_PLACEHOLDER,
        full_content
    )
    # standardize spacing between classes to 3 new lines, not effecting decorators
    full_content = re.sub(r"\n+(?=(class |[A-Z]\w+\s*=\s*))", "\n\n\n", full_content)
    full_content = full_content.replace(DECORATOR_PLACEHOLDER, "\n")

...
def main():
    ...
    try:
        ...
        print(f"❌ ERROR: Could not find command name list file. Please run the schema generator first. ({e})")
        return

    try:
        content = official_paths.CLEANED_TYPED_DICTS.read_text("utf-8")
    except FileNotFoundError:
        print(f"❌ ERROR: {official_paths.CLEANED_TYPED_DICTS} not found. Please run the cleaner script first.")
        return
    ...
    if not definitions_in_order:
        print("❌ ERROR: No definitions found in input file.")
        return
    ...
    print(f"✅ Sorted definitions: {len(base_model_blocks)} base | {len(command_model_blocks)} command")
    ...
    print("\n⚙️  Processing base TypedDicts (types.py)...")
    ...
    print(f"✅ Successfully wrote {len(base_model_blocks)} definitions to {official_paths.FINAL_TYPED_DICT_TYPES}")

    # --- Write `commands.py` (Command TypedDicts) ---
    print("\n⚙️  Processing command TypedDicts (commands.py)...")
    needed_imports = set()
    ...
    print(
        f"✅ Successfully wrote {len(command_model_blocks)} definitions to {official_paths.FINAL_TYPED_DICT_COMMANDS}"
    )

...
def remove_unused_imports(content: str) -> str:
    ...
    used_imports = []
    if re.search(r"\bAny\b", body_content):
        used_imports.append("Any")
    if re.search(r"\bTypeAlias\b", body_content):
        used_imports.append("TypeAlias")
    if re.search(r"\bLiteral\b", body_content):
        used_imports.append("Literal")
    if re.search(r"\bTypedDict\b", body_content):
        used_imports.append("TypedDict")
    if re.search(r"\bUnion\b", body_content):
        used_imports.append("Union")
...
def main():
    ...
    try:
        ...
        if not tapir_paths.BASE_MODEL_NAMES_OUTPUT.exists():
            with open(tapir_paths.BASE_MODEL_NAMES_OUTPUT, "w", encoding="utf-8") as f:
                json.dump([], f)
        if not tapir_paths.COMMAND_MODELS_NAMES_OUTPUT.exists():
            with open(tapir_paths.COMMAND_MODELS_NAMES_OUTPUT, "w", encoding="utf-8") as f:
                json.dump([], f)
    ...
    # Filter out empty blocks, metadata blocks, and clean code-gen headings
    ...
    for block in re.split(r"\n\n\n", content):
        ...
        name = get_definition_name(block_stripped)
        if name:
            # Strip loose generated lines (import headings) inside blocks
            clean_lines = [
                line for line in block_stripped.splitlines() if not line.strip().startswith(("from ", "import ", "###"))
            ]
            definitions_in_order.append("\n".join(clean_lines).strip())

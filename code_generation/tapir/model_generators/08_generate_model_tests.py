...
from multiconn_archicad.models.tapir import commands as tapir_models
...
def main():
    ...
    Pydantic models and their corresponding TypedDicts.
    """
...
pytestmark = pytest.mark.generated
    ...
    for model_name in sorted(command_model_names):
        ...
        # Determine validation logic: Use TypeAdapter for Unions, model_validate for Classes
        try:
            model_obj = getattr(tapir_models, model_name)
            if is_union(model_obj):
                pydantic_val_call = f"TypeAdapter(models.{model_name}).validate_python(data)"
            else:
                pydantic_val_call = f"models.{model_name}.model_validate(data)"
        except AttributeError:
            # Fallback if the model is not found in tapir_models
            pydantic_val_call = f"models.{model_name}.model_validate(data)"

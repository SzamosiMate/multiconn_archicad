...
def remove_unused_imports(content: str) -> str:
    ...
    leaving the body of the file and its spacing untouched.
    """
    # 1. Find the boundary between header and body.
    header_boundary_match = re.search(r"^(### This file is automatically generated.*?###\n)", content, re.MULTILINE)
    if not header_boundary_match:
        return content  # Safeguard: if no header, return as-is.

    header_end_pos = header_boundary_match.end()
    header = content[:header_end_pos]
    body = content[header_end_pos:]

    # 2. Process `typing` imports
    used_typing = []
    if re.search(r"\bAny\b", body): used_typing.append("Any")
    if re.search(r"\bList\b", body): used_typing.append("List")
    if re.search(r"\bLiteral\b", body): used_typing.append("Literal")
    if re.search(r"\bTypeAlias\b", body): used_typing.append("TypeAlias")
    if re.search(r"\bUnion\b", body): used_typing.append("Union")
    ...
    # 3. Process `pydantic` imports
    used_pydantic = []
    if re.search(r"\bBaseModel\b", body): used_pydantic.append("BaseModel")
    if re.search(r"\bConfigDict\b", body): used_pydantic.append("ConfigDict")
    if re.search(r"\bField\b", body): used_pydantic.append("Field")
    if re.search(r"\bRootModel\b", body): used_pydantic.append("RootModel")
    ...
    # 4. Process single imports
    if "from uuid import UUID" in header and not re.search(r"\bUUID\b", body):
        header = re.sub(r"^from uuid import UUID\n", "", header, flags=re.MULTILINE)
    if "from enum import Enum" in header and not re.search(r"\bEnum\b", body):
        header = re.sub(r"^from enum import Enum\n", "", header, flags=re.MULTILINE)

    # 5. Clean up extra blank lines in the header and return

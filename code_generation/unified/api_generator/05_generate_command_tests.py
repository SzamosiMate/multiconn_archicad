...
import textwrap
import inspect
from typing import Any, Dict, Set, get_origin, Union
from types import UnionType

from multiconn_archicad.models.tapir import commands as tapir_commands, types as tapir_types
from multiconn_archicad.models.official import commands as official_commands, types as official_types
...
def patch_recursive_schemas(definitions: dict, model_name_to_test: str, source: str) -> dict:
    ...
    if "KeynoteFolderDetails" in patched_defs and "properties" in patched_defs["KeynoteFolderDetails"]:
        # subFolders recurses but is required, so bound it to an always-empty array instead of
        # dropping it - otherwise the generated data no longer satisfies the model.
...
class TestGenerator:
    ...
    def _get_model_object(self, name: str, source: str) -> Any:
        """Helper to find the actual Python object for a model name."""
        try:
            return getattr(MODEL_MAP[source]["commands"], name)
        except AttributeError:
            return getattr(MODEL_MAP[source]["types"], name)
        ...
        if has_result:
            if alias_prop:
                ...
                if is_union(result_obj):
                    body_lines.append(f"expected = TypeAdapter(commands.{result_model_name}).validate_python(mock_response)")
                else:
                    body_lines.append(f"expected = commands.{result_model_name}.model_validate(mock_response)")
                body_lines.append("assert result == expected")
        else:
    ...
    def _write_test_files(self):
        ...
        utility_functions_source = inspect.getsource(camel_to_snake)

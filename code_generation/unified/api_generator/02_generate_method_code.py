...
# --- Helper Functions with Dependency Tracking ---


def get_model_class(model_name: str, source: str, dependencies: dict) -> Any | None:
    """Safely retrieves a model class and records its usage."""
    try:
...
def _build_docstring(description: str, param_docs: list[str]) -> str:
    """Assembles the full method docstring."""
    ...
    if params_model:
        ...
        body_lines.extend(
            [
                ...
                f"response_dict = self._core.{core_call_method}(",
                f'    "{original_command_name}",',
                "    validated_params.model_dump(by_alias=True, exclude_none=True)",
                ")",
            ]
        )
    else:
        body_lines.append(f'response_dict = self._core.{core_call_method}("{original_command_name}")')
...
def main():
    """Main script for Stage 2 of the pipeline."""

    ...
    with ui.scroll_area().classes("h-200"):
        if app_state.first_port:
            for port, project_name in app_state.instance_ids.items():
                ui.checkbox(
                    project_name,
                    on_change=lambda e, i=port: app_state.connect_or_disconnect(i, e.value),
                ).props("dense")
        else:
            no_archicad()
...
def no_archicad() -> None:
    ui.label("Found no running ArchiCAD to connect to. Please start Archicad and hit Refresh")
...
def set_global_context() -> None:
    ...
    ui.colors(primary="#28323C", secondary="blue-grey-1", positive="#53B689", accent="#111B1E")
...
def index():
    ...
    with ui.card(align_items="stretch").classes("w-full h-full").tight():
        with ui.tabs().classes("w-full").props("active-bg-color=primary active-color=white dense") as tabs:
            single = ui.tab("Single")
            multi = ui.tab("Multiple")
        with (
            ui.tab_panels(tabs, value=single).bind_value(app_state, target_name="run_mode").classes("p-0 gap-0 h-full")
        ):
    ...
    (ui.button("Set Parameters", on_click=set_parameters).classes("w-full").bind_enabled_from(app_state, "parameters"))
